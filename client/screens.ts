...
export class Screens {
  ...
  setCarCatalog(names: string[], thumbs: string[]): void {
    ...
    if (this.visible && this.phase === 'car_select') this.rerenderCarSelect();
  }
  /** Progressive thumbnails: a portrait finished — store it and live-swap that tile's <img>. */
  setCarThumb(i: number, url: string): void {
    ...
    else if (this.visible && this.phase === 'car_select') this.rerenderCarSelect();
  }
  ...
  private rerenderCarSelect(): void {
  ...
  private head(title: string, sub: string): string {
    return `
      <div class="scr-head">
        <div class="scr-eyebrow"><img src="${BUG}" alt="">Voice Racer</div>
        <div class="scr-title">${esc(title)}</div>
  ...
  private chips(players: LobbyPlayer[]): string {
    ...
    const chips = players.map(p => {
      ...
      const car = p.carIndex !== null ? esc(this.carNames[p.carIndex] ?? `Car ${p.carIndex + 1}`) : '…';
      return `
