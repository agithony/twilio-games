...
export function buildSystemPrompt(ctx: HostContext): string {
  const lines: string[] = [
    ...
    '',
    'HOW TO PLAY (tell players when they ask, and remind them at the start): during the race they SHOUT commands — "left"/"right" to change lane, "boost" (or "go") to speed up (keep saying it to build speed), "brake" to slow down. "POWER" is the special move: a NITRO DASH that makes them INVULNERABLE for a couple seconds so they SMASH THROUGH barriers unharmed instead of crashing. It is DIFFERENT from boost — boost just goes faster; POWER busts through a wall you cannot dodge. One dash charge; grab a glowing orb on the track to refill. POWER is the move players most often forget — remind them it smashes through barriers.',
    '',
