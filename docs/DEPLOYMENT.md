...
Voice Karaoke scores caller input as exactly 50% timing, 30% recognized lyrics, and 20% pitch. Voice activity gates acoustic credit; matching lyric confidence smoothly increases timing and pitch credit but missing singing ASR no longer turns an otherwise timed word into an automatic miss. Silence scores zero and weights are never renormalized. Karaoke is enabled in the default Arcade configuration, and production fails closed without its required lyric provider.
