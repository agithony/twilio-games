...
| `ARCADE_STANDALONE_VOICE_ENABLED` | Literal `true` | Lets standalone play route calls to the game currently open on the shared display |
...
| `TWILIO_WHATSAPP_CONTENT_SID_STATION_*_{EN_US,PT_BR}` | GitHub repository variables | Approved localized WhatsApp templates; call-now uses a static Phone CTA and is also used inside the 24-hour session window, while the other templates are needed outside it |
